...
基于NMF（非负矩阵分解）的盲源分离常将模型简化为声音信号的线性叠加。矩阵表示为：X=BG，其中X为混合源信息，B为基源信息，G为时变增益（time-varying gain）。NMF的任务就是在只有矩阵X的情况下获取B，G。  
其中，X往往为m*t的矩阵，也即每一行为频率点/段，每一列为时间帧。因此相应地，B为m*n矩阵，G为n*t矩阵，其中n可代表分离源个数。而在我们的研究中，m一般取88。  
